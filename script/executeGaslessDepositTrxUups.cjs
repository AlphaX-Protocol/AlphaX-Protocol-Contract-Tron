...
async function main() {
  ...
  if (!USER_PRIVATE_KEY || !DEPLOYER_PRIVATE_KEY || !RECIPIENT_ADDRESS) {
    console.error('Missing env: DEPLOYER_PRIVATE_KEY, RECIPIENT_ADDRESS.');
    process.exit(1);
  }

  // --- Load UUPS deployed addresses ---
  ...
  try {
    ...
    console.log(`Loaded addresses from ${deployedPath}`);
  } catch (e) {
    console.error(`Error loading ${deployedPath}. Run deployUups.cjs first.`);
    process.exit(1);
  }
  ...
  if (!GAS_FREE_CONTROLLER_ADDRESS || !DEX_VAULT_ADDRESS) {
    console.error('Missing controllerAddress or dexVaultProxy in deployed-addresses-uups.json.');
    process.exit(1);
  }

  const tronWebUser = new TronWeb(
    networkConfig.fullNode, networkConfig.solidityNode, networkConfig.eventServer, USER_PRIVATE_KEY
  );
  ...
  if (!userGasFreeAccountAddress) {
    console.error('User GasFreeAccount address unknown. Deploy an account first (via deployUups.cjs).');
    process.exit(1);
  }
  if (!userGasFreeAccountAddress.startsWith('T')) {
    userGasFreeAccountAddress = deployedAddresses.userGasFreeAccountAddress || null;
    if (!userGasFreeAccountAddress) {
      console.error('Invalid GasFreeAccount address from factory.');
      process.exit(1);
    }
  }
  ...
  console.log('\n--- Signing PermitTransfer (TRX deposit to UUPS proxy vault) ---');
  ...
  try {
    ...
    console.error('estimateEnergy failed. Aborting:', estimateErr.message || estimateErr);
    process.exit(1);
  }
  ...
  console.log('DEX Vault (proxy):', vaultNum, 'sun');
